...
for addr in $ips
do
    revers=`eval echo $addr | tr ' ' '\n' | sed 's/\(.*\)\.\(.*\)\.\(.*\)\.\(.*\)/\4.\3.\2.\1/'`;
    #host $revers.b.barracudacentral.org | grep -v NXDOMAIN && echo | echo "IP: "$addr
    host $revers.b.barracudacentral.org | grep -v NXDOMAIN &&  echo $addr
done
